...
################################################################
# Ryzsh10k Theme
# https://github.com/ryzsh/ryzsh10k
#
# Forked from Ryzsh9k Theme
# https://github.com/bhilburn/ryzsh9k
#
...
(( $+__r9k_root_dir )) || typeset -gr __r9k_root_dir=${RYZSH9K_INSTALLATION_DIR:-${${(%):-%x}:A:h}}
(( $+__r9k_intro )) || {
...
() {
  ...
  if [[ $ZSH_VERSION == (5.<1->*|<6->.*) ]]; then
    if [[ -w $__r9k_root_dir && -w $__r9k_root_dir/internal && -w $__r9k_root_dir/gitstatus ]]; then
      local f
      for f in $__r9k_root_dir/{ryzsh9k.zsh-theme,ryzsh10k.zsh-theme,internal/r10k.zsh,internal/icons.zsh,internal/configure.zsh,internal/worker.zsh,internal/parser.zsh,gitstatus/gitstatus.plugin.zsh,gitstatus/install}; do
        [[ $f.zwc -nt $f ]] && continue
