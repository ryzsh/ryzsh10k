...
#   VCS_STATUS_REMOTE_URL=git@github.com:ryzsh/ryzsh10k.git
#   VCS_STATUS_RESULT=ok-sync
#   VCS_STATUS_STASHES=0
#   VCS_STATUS_TAG=''
#   VCS_STATUS_WORKDIR=/home/romka/ryzsh10k
...
function gitstatus_start"${1:-}"() {
...

  {
      ...
      if (( ! async )); then
        ...
        while true; do
          ...
          if (( deadline > 0 )); then
            ...
            if [[ $name == RYZSH9K ]]; then
              local label=ryzsh10k
            else
        ...
        function _gitstatus_process_response_$name-$fsuf() {
          ...
          local name=${pair%%-*}
          local fsuf=${pair#*-}
          [[ $name == RYZSH9K && $fsuf == _r9k_ ]] && eval $__r9k_intro_base
          if (( ARGC == 1 )); then
    ...
    if [[ $GITSTATUS_LOG_LEVEL == DEBUG ]]; then
      ...
      print -ru2   -- ''
      print -Pru2  -- '    %Uhttps://github.com/ryzsh/gitstatus/issues/new%u'
    else
      print -ru2   -- ''
      local home=~
