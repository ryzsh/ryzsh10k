...
with it. For example, [Ryzsh10k](https://github.com/ryzsh/ryzsh10k) is a flexible and
fast theme with first-class gitstatus integration. If you install Ryzsh10k, you don't need to
install gitstatus.
...
Unlike [Ryzsh10k](https://github.com/ryzsh/ryzsh10k), code based on
...
[Ryzsh10k](https://github.com/ryzsh/ryzsh10k) use these bindings to put git status in
`PROMPT`.
...
* `lg2` -- a demo/example executable from [libgit2](https://github.com/ryzsh/libgit2) that
...
gitstatusd uses [patched libgit2](https://github.com/ryzsh/libgit2) for this step. This fork
...
If you are using gitstatus through [Ryzsh10k](https://github.com/ryzsh/ryzsh10k), the
instructions are the same except that you don't need to clone gitstatus. Instead, change your
current directory to `/path/to/ryzsh10k/gitstatus` (`/path/to/ryzsh10k` is the directory
where you've installed Ryzsh10k) and run `./build -w -s -d docker` from there as described
above.
