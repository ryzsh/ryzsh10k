...
- support states in SHOW_ON_COMMAND: RYZSH9K_SEGMENT_STATE_SHOW_ON_COMMAND='...'

---

add RYZSH9K_${SEGMENT}_${STATE}_SHOW_IN_DIR='pwd_pattern'; implement the same way as
SHOW_ON_UPGLOB. how should it interact with RYZSH9K_${SEGMENT}_DISABLED_DIR_PATTERN?
...
- add faq: how to customize directory shortening? mention RYZSH9K_DIR_TRUNCATE_BEFORE_MARKER,
RYZSH9K_DIR_MAX_LENGTH and co., and truncate_to_last.

---

fix a bug in zsh: https://github.com/ryzsh/ryzsh10k/issues/502. to reproduce:
...
Add `RYZSH9K_{LEFT,RIGHT}_SEGMENT_MIRROR_SEPARATOR`. If set, left segments get separated with
`RYZSH9K_LEFT_SEGMENT_SEPARATOR` followed by `RYZSH9K_LEFT_SEGMENT_MIRROR_SEPARATOR`.
...
`RYZSH9K_{LEFT,RIGHT}_SEGMENT_MIRROR_SEPARATOR`.
`RYZSH9K_{LEFT,RIGHT}_SUBSEGMENT_SEPARATOR` is unused.
...
```text
time ( repeat 1000 [[ -z "${parameters[(I)RYZSH9K_*~(RYZSH9K_MODE|RYZSH9K_CONFIG_FILE)]}" ]] )
user=0.21s system=0.05s cpu=99% total=0.264

time ( repeat 1000 [[ -z "${parameters[(I)RYZSH9K_*]}" ]] )
...
Respect `RYZSH9K_VCS_STASH_ICON` overrides but not anything with segment name or state.

Define `RYZSH9K_VCS_*` parameters in config templates for all symbols used in
...
Add a screen to the wizard asking whether to set `RYZSH9K_VCS_DISABLED_WORKDIR_PATTERN='~'`.
