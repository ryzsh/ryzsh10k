...
function quit() {
  ...
  print
  if (( force )); then
    flowing Ryzsh10k configuration wizard has been aborted. To run it again, type:
    print -P ""
    print -P "  %2Fr10k%f %Bconfigure%b"
    print -P ""
  else
    flowing                                                                        \
      Ryzsh10k configuration wizard has been aborted. It will run again       \
      next time unless you define at least one Ryzsh10k configuration option. \
      To define an option that does nothing except for disabling Ryzsh10k     \
      configuration wizard, type the following command:
    print -P ""
    print -P "  %2Fecho%f %3F'RYZSH9K_DISABLE_CONFIGURATION_WIZARD=true'%f >>! $__r9k_zshrc_u"
    print -P ""
    flowing To run Ryzsh10k configuration wizard right now, type:
    print -P ""
    print -P "  %2Fr10k%f %Bconfigure%b"
    print -P ""
  fi
    ...
    while true; do
      while true; do
        ...
        if (( wizard_columns < __r9k_wizard_columns )); then
          ...
          print
          flowing Make terminal window %Bwider%b or press %BCtrl-C%b to abort Ryzsh10k configuration wizard.
        elif (( wizard_lines < __r9k_wizard_lines )); then
          clear
          flowing -c %1FNot enough vertical space.%f
          print
          flowing Make terminal window %Btaller%b or press %BCtrl-C%b to abort Ryzsh10k configuration wizard.
        else
          break
        fi
...
function ask_icon_padding() {
  if [[ $RYZSH9K_MODE == (powerline|compatible|ascii) ]]; then
    RYZSH9K_ICON_PADDING=none
    return 0
  fi
...
function ask_charset() {
  [[ $style == (lean*|classic|rainbow) && $RYZSH9K_MODE != ascii ]] || return 0
...
function ask_extra_icons() {
  if [[ $style == pure || $RYZSH9K_MODE == (powerline|compatible|ascii) ]]; then
    return 0
  fi
...
function ask_heads() {
  if [[ $style != (classic|rainbow) || $RYZSH9K_MODE == ascii ]]; then
    return 0
  fi
  local extra
...
function ask_frame() {
  if [[ $style != (classic|rainbow|lean*) || $num_lines != 2 || $RYZSH9K_MODE == ascii ]]; then
    return 0
  fi
...
function print_instant_prompt_link() {
  local link='https://github.com/ryzsh/ryzsh10k#instant-prompt'
...
function ask_config_overwrite() {
  ...
  add_widget 0 flowing -c Ryzsh10k config file already exists.
...
function generate_config() {
  ...
  function sub() {
    lines=("${(@)lines/#(#b)([[:space:]]#)typeset -g RYZSH9K_$1=*/$match[1]typeset -g RYZSH9K_$1=$2}")
  }
  ...
  if [[ $style == pure ]]; then
    ...
    sub MODE $RYZSH9K_MODE

    sub ICON_PADDING $RYZSH9K_ICON_PADDING

    if [[ $RYZSH9K_MODE == compatible ]]; then
    ...
    if [[ $RYZSH9K_MODE == (compatible|powerline) ]]; then
      uncomment 'typeset -g RYZSH9K_LOCK_ICON'
      sub LOCK_ICON "'∅'"
      uncomment 'typeset -g RYZSH9K_NORDVPN_VISUAL_IDENTIFIER_EXPANSION'
      sub NORDVPN_VISUAL_IDENTIFIER_EXPANSION "'nord'"
      uncomment 'typeset -g RYZSH9K_RANGER_VISUAL_IDENTIFIER_EXPANSION'
      sub RANGER_VISUAL_IDENTIFIER_EXPANSION "'▲'"
      uncomment 'typeset -g RYZSH9K_YAZI_VISUAL_IDENTIFIER_EXPANSION'
      sub YAZI_VISUAL_IDENTIFIER_EXPANSION "'▲'"
      uncomment 'typeset -g RYZSH9K_KUBECONTEXT_DEFAULT_VISUAL_IDENTIFIER_EXPANSION'
      sub KUBECONTEXT_DEFAULT_VISUAL_IDENTIFIER_EXPANSION "'○'"
      uncomment 'typeset -g RYZSH9K_AZURE_VISUAL_IDENTIFIER_EXPANSION'
      sub AZURE_VISUAL_IDENTIFIER_EXPANSION "'az'"
      uncomment 'typeset -g RYZSH9K_AWS_EB_ENV_VISUAL_IDENTIFIER_EXPANSION'
      sub AWS_EB_ENV_VISUAL_IDENTIFIER_EXPANSION "'eb'"
      uncomment 'typeset -g RYZSH9K_BACKGROUND_JOBS_VISUAL_IDENTIFIER_EXPANSION'
      sub BACKGROUND_JOBS_VISUAL_IDENTIFIER_EXPANSION "'≡'"
    fi

    if [[ $RYZSH9K_MODE == (awesome-patched|awesome-fontconfig) && $cap_python == 0 ]]; then
      uncomment 'typeset -g RYZSH9K_VIRTUALENV_VISUAL_IDENTIFIER_EXPANSION'
      uncomment 'typeset -g RYZSH9K_ANACONDA_VISUAL_IDENTIFIER_EXPANSION'
      uncomment 'typeset -g RYZSH9K_PYENV_VISUAL_IDENTIFIER_EXPANSION'
      uncomment 'typeset -g RYZSH9K_PYTHON_ICON'
    ...
    if [[ -n ${(j::)extra_icons} ]]; then
      ...
      uncomment 'typeset -g RYZSH9K_DIR_CLASSES'
      uncomment 'typeset -g RYZSH9K_VCS_VISUAL_IDENTIFIER_EXPANSION'
      uncomment 'typeset -g RYZSH9K_COMMAND_EXECUTION_TIME_VISUAL_IDENTIFIER_EXPANSION'
      uncomment 'typeset -g RYZSH9K_TIME_VISUAL_IDENTIFIER_EXPANSION'
    ...
    if [[ -n ${(j::)prefixes} ]]; then
      uncomment 'typeset -g RYZSH9K_VCS_PREFIX'
      uncomment 'typeset -g RYZSH9K_COMMAND_EXECUTION_TIME_PREFIX'
      uncomment 'typeset -g RYZSH9K_CONTEXT_PREFIX'
      uncomment 'typeset -g RYZSH9K_KUBECONTEXT_PREFIX'
      uncomment 'typeset -g RYZSH9K_TIME_PREFIX'
      uncomment 'typeset -g RYZSH9K_TOOLBOX_PREFIX'
  ...
  local header=${(%):-"# Generated by Ryzsh10k configuration wizard on %D{%Y-%m-%d at %H:%M %Z}."}$'\n'
  header+="# Based on ryzsh/ryzsh10k/config/r10k-${style//_/-}.zsh"
...
function change_zshrc() {
...

  {
    ...
    if (( !zshrc_has_instant_prompt )); then
      >>$tmp print -r -- "# Enable Ryzsh10k instant prompt. Should stay close to the top of ${(%)__r9k_zshrc_u}.
...
function check_zshrc_integration() {
  ...
  local h10='$RYZSH9K_CONFIG_FILE'
  local h11='"$RYZSH9K_CONFIG_FILE"'
...
() {
  ...
  print -P ""
  flowing                                                                          \
      Ryzsh10k configuration file "($__r9k_cfg_path_u)" was not sourced. This \
      ...
      there are no errors, try running Ryzsh10k configuration wizard:
  print -P ''
  print -P '  %2Fr10k%f %Bconfigure%b'
  print -P ''
  flowing                                                                              \
      If you do nothing, you will see this message again when you start zsh. You can   \
      suppress it by defining %BRYZSH9K_DISABLE_CONFIGURATION_WIZARD=true%b in    \
      $__r9k_zshrc_u.
  print -P ''
  return 1
} || return
...
while true; do
  ...
  local RYZSH9K_MODE= RYZSH9K_ICON_PADDING=moderate style= config_backup= config_backup_u=
  ...
  if [[ -o multibyte && $TERM != (dumb|linux) && $langinfo[CODESET] == (utf|UTF)(-|)8 ]]; then
    ...
    if [[ $AWESOME_GLYPHS_LOADED == 1 ]]; then
      RYZSH9K_MODE=awesome-mapped-fontconfig
    else
      ask_lock '\uF023' || continue
      if (( ! cap_lock )); then
        ...
        if (( cap_lock )); then
          ...
          if (( cap_diamond )); then
            RYZSH9K_MODE=powerline
          else
            ask_quotes || continue
            (( cap_quotes )) && RYZSH9K_MODE=compatible || RYZSH9K_MODE=ascii
          fi
        fi
      elif (( ! cap_diamond )); then
        RYZSH9K_MODE=awesome-fontconfig
      else
        ...
        if (( cap_arrow )); then
          RYZSH9K_MODE=nerdfont-v3
        else
          ask_arrow '\uFC35' "Let's try another one." || continue
          if (( cap_arrow )); then
            RYZSH9K_MODE=nerdfont-complete
          else
            RYZSH9K_MODE=awesome-fontconfig
            ask_python || continue
          fi
  ...
  if [[ $RYZSH9K_MODE == powerline ]]; then
    options+=powerline
  elif (( cap_diamond )); then
    options+="$RYZSH9K_MODE + powerline"
  else
    options+="$RYZSH9K_MODE"
  fi
...
if (( !in_z4h_wizard )); then
  print -rP ""
  flowing +c File feature requests and bug reports at "$(href https://github.com/ryzsh/ryzsh10k/issues)"
  print -rP ""
fi
