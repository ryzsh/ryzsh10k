...
function _r9k_init_icons() {
  [[ -n ${RYZSH9K_MODE-} || ${langinfo[CODESET]} == (utf|UTF)(-|)8 ]] || local RYZSH9K_MODE=ascii
  [[ $_r9k__icon_mode == $RYZSH9K_MODE/$RYZSH9K_LEGACY_ICON_SPACING/$RYZSH9K_ICON_PADDING ]] && return
  typeset -g _r9k__icon_mode=$RYZSH9K_MODE/$RYZSH9K_LEGACY_ICON_SPACING/$RYZSH9K_ICON_PADDING

  if [[ $RYZSH9K_LEGACY_ICON_SPACING == true ]]; then
    local s=
    local q=' '
  else
    local s=' '
  ...
  case $RYZSH9K_MODE in
    ...
    'awesome-mapped-fontconfig')
      ...
      if [ -z "$AWESOME_GLYPHS_LOADED" ]; then
          echo "Ryzsh9k warning: Awesome-Font mappings have not been loaded.
          Source a font mapping in your shell config, per the Awesome-Font docs
          (https://github.com/gabrielelana/awesome-terminal-fonts),
          Or use a different Ryzsh9k font configuration.";
      fi
      icons=(
        RULER_CHAR                     '\u2500'                                       # ─
        LEFT_SEGMENT_SEPARATOR         '\uE0B0'                                       # 
  ...
  if [[ $RYZSH9K_ICON_PADDING == none && $RYZSH9K_MODE != ascii ]]; then
    icons=("${(@kv)icons%% #}")
